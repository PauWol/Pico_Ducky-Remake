...
def randomMouseMove(time_s = 0.5):
    x = random.randint(-800,800)
    y = random.randint(-800,800)
    mouse.move(x,y,5)
    time.sleep(time_s)

def move_mouse(x=1000,y=1000,speed=5):
    mouse.move(x,y,5)

def clicker(typ = 1):
    if typ == 'left' or typ == 1:
        mouse.click(Mouse.LEFT_BUTTON)
    if typ == 'right' or typ == 2:
...
def parseLine(line):
    ...
    elif(line[0:10] == "VOLUMEDOWN"):
        for i in range(int(line[11:])):
            cc.send(ConsumerControlCode.VOLUME_DECREMENT)
    elif(line[0:7] == "LIGHTUP"):
        for i in range(int(line[8:])):
            cc.send(ConsumerControlCode.BRIGHTNESS_INCREMENT)
    elif(line[0:9] == "LIGHTDOWN"):
        for i in range(int(line[10:])):
